package com.dtu.mark.carelink_android.Decoding;

/**
 * Created by mark on 08/11/14.
 */
public class Lib {
    private static int[] CRC16CCITT_lookup = { 0, 4129, 8258, 12387, 16516, 20645, 24774,
    ...
    private static int[] CRC8_lookup = { 0, 155, 173, 54, 193, 90, 108, 247, 25, 130, 180, 47,
    ...
    public static int bangLong(byte a, byte b, byte c, byte d) {
        return a << 24 | b << 16 | c << 8 | d;
    }

    public static int bangInt(byte a, byte b) {
        return a << 8 | b;
    }

    public static int highByte(byte arg) {
        return ((arg >> 8) & 0xFF);
    }

    public static int lowByte(byte arg) {
        return (arg & 0xFF);
    }

    public static int CRC16CCITT(byte[] block) {
        int tmp;
        int result = 65535;
        for (int i = 0; i < block.length; i++) {
            tmp = block[i] ^ result >> 8;
            result = (CRC16CCITT_lookup[tmp] ^ result << 8) & 0xFFFF;
        }
        return result;
    }

    public static int CRC8(byte[] block) {
        int result = 0;
        for (int i = 0; i < block.length; i++) {
            result = CRC8_lookup[result ^ block[i] & 0xFF];
        }
        return result;
    }
}
